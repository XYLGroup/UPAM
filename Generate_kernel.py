...
if __name__ == '__main__':
    ...
    i = 0
    for sigma1 in np.arange(0.175 * scale_factor, min(2.5 * scale_factor, 10) + 0.3, 0.3):
        for sigma2 in np.arange(0.175 * scale_factor, min(2.5 * scale_factor, 10) + 0.3, 0.3):
            for theta in np.arange(0, np.pi, 0.2):
                kernel = gen_kernel_fixed(np.array([K_N, K_M]),
                                          np.array([scale_factor, scale_factor]),
                                          sigma1, sigma2, theta, 0)
                mpimg.imsave(os.path.join(kernelpath, str(i) + '.png'), kernel)

                # torch.save(torch.from_numpy(kernel), os.path.join(kernelpath, str(i) + '.pth'))
                i += 1
